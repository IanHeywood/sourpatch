...
chans = 1055
fitslist = sorted(glob.glob('zoom*fits'))
nfits = len(fitslist)
chunks = int(numpy.ceil(nfits/chans))
overlap = 55

ranges = [fitslist[i:i+chans] for i in range(0, len(fitslist), chans-overlap)]

for i in range(0,len(ranges)):
	chunklist = ranges[i]
	start_chan = chunklist[0].split('-')[-1]
	end_chan = chunklist[-1].split('-')[-1]
	...
	print('Chunk %s has %s planes (chans %s to %s)' % (i,len(chunklist),start_chan,end_chan))
