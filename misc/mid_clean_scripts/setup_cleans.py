...
for ptg in ptgs:
    ...
    for myms in mslist:
        ...
        opname = opdir+'/img_'+myms+'_r0p5'

        syscall = 'singularity exec '+container+' wsclean -log-time '
        syscall += '-abs-mem 225 -parallel-reordering 12 -name '+opname+' '
        syscall += '-tempdir '+opdir+' -data-column DATA -parallel-deconvolution 2048 '
        syscall += '-field 0 -size 4096 4096 -scale 2.0asec -use-wgridder -weight briggs 0.5 '
