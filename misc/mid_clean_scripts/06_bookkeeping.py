...
#	print(ss,chans)
	...
	for subset in subsets:
		...
		for fitsfile in fitslist:
			fitschan = int(fitsfile.split('r0p5-')[-1].split('-')[0])
			subset_chans.append(fitschan)
		subset_chans = numpy.array(subset_chans)+ch0
		for i in range(0,len(fitslist)):
			newimgname = fitslist[i].split('r0p5')[0]+'r0p5-'+str(subset_chans[i]).zfill(4)+'-image.fits'
			...
			for suffix in suffixes:
				...
				if os.path.isfile(ff):
					newname = ff.split('r0p5')[0]+'r0p5-'+str(subset_chans[i]).zfill(4)+'-'+suffix
