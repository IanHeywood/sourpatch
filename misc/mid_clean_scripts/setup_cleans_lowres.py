...
for ptg in ptgs:
    ...
    mslist = sorted(glob.glob('*0769-1152*.mms'))
    msid = 0
    for myms in mslist:
...
#        maskcube = glob.glob('/idia/projects/mightee/ianh/HI/*/MID/images/pony.o*/filtered/cube*/*'+field+'*'+chans+'*fits')[0]
        maskcube = glob.glob('out*'+myms+'*/pony.o*/filtered/*cube.fits')[0]
        ...
        syscall += '-tempdir '+opdir+' -data-column DATA  '
        syscall += '-field 0 -size 1024 1024 -scale 8.0asec -no-update-model-required -use-wgridder -weight briggs 1.0 -taper-gaussian 40 '
